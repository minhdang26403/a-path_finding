...
def algorithm(draw, grid, start, end):
    ...
    while not open_set.empty():
        ...
        if current == end:
            ...
            return True
        for neighbor in current.neighbors:          # temp_g_score = g_score[current] + w(current, neighbor)
            temp_g_score = g_score[current] + 1     # w is 1 in this case
            if g_score[neighbor] > temp_g_score:    # d[v] > d[u] + w(u,v) => relax edge
